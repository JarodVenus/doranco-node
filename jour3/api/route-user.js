...
route.delete("/:id", async (request , response) => {
    ...
    if(!isValidObjectId(id)) return response.status(400).json({msg : `id ${id} est invalide`})
    // si ok on continue 
    ...
    if(!profilASupprimer) return response.status(404).json({msg : `Profil introuvable avec l'id mentionné : ${id}`})
    // si ok => message "profil supprimé" STOP 
    response.json({msg : `profil ${id} est supprimé`})
})
// test 
